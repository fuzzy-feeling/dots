...
arrowleft="^fn(hack-10)^fn()"
arrowright="^fn(hack-10)^fn()"
arrowrightsmall="^fn(hack-10)^fn()"
arrow_left_small="^fn(hack-10)^fn()"
...

{
    ...
    while true ; do
        ...
        for i in "${tags[@]}" ; do
            case ${i:0:1} in
                '#')
                    color="^bg($selbg)^fg($selfg)"
		    arrow_color="^bg($selfg)^fg($selbg)"
                    ;;
                '+')
                    color="^bg(#9CA668)^fg(#141414)"
		    arrow_color="^bg($selbg)^fg($selfg)"
                    ;;
                ':')
                    color="^bg()^fg(#ffffff)"
		    arrow_color="^bg($selbg)^fg($selfg)"
	    ...
	    echo -n "$arrow_color$arrowright$color"
        done
        echo -n "$separator"
